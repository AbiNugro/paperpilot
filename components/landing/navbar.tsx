    ...
    <header className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-5">
      ...
      >
        <div className="flex h-14 items-center justify-between">
          <Link href="#top" aria-label={t("home")} className="rounded-lg" onClick={() => setOpen(false)}>
            <Brand priority />
          </Link>
          ...
          <div className="overflow-hidden">
            <div className="border-t border-black/[.07] px-1 pb-4 pt-2">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                <ButtonLink href="/login" onClick={() => setOpen(false)} variant="secondary">{t("signIn")}</ButtonLink>
                <ButtonLink href="/dashboard" onClick={() => setOpen(false)}>{t("getStarted")}</ButtonLink>
              </div>
              <div className="mt-2 flex justify-end"><LanguageSwitcher variant="landing" /></div>
            </div>
          </div>
        </div>
      </nav>
