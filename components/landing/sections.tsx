    ...
    <section id="product" className="py-16 sm:py-20">
      <div className="mx-auto max-w-[1180px] px-5">
        ...
        <div className="mt-14 grid items-stretch gap-4 lg:grid-cols-[1fr_72px_1fr] lg:gap-0">
          <div className="rounded-[18px] bg-[#f1f2f4] p-5 shadow-[inset_0_0_0_1px_rgba(15,23,42,.055)] sm:p-7 lg:rounded-r-none">
            ...
            <div className="mt-6 grid grid-cols-2 gap-2">
              {beforeItems.map(([Icon, label]) => (
                <div key={label as string} className="flex min-h-20 items-center gap-3 rounded-[11px] bg-white px-4 text-sm font-medium text-[#606876] shadow-[0_0_0_1px_rgba(15,23,42,.055)]">
          ...
          <div className="rounded-[18px] bg-[#eef4ff] p-5 shadow-[inset_0_0_0_1px_rgba(37,99,235,.09)] sm:p-7 lg:rounded-l-none">
            <div className="flex items-center justify-between">
            ...
            <div className="mt-6 space-y-2.5">
              {afterItems.map(([Icon, label], index) => (
                <div key={label as string} className="surface-interactive flex min-h-[60px] items-center gap-3 rounded-[11px] bg-white px-4">
...
function UploadPreview() {
  return (
    <div className="surface h-full w-full rounded-[17px] bg-white p-2">
      <div className="h-full rounded-[11px] bg-[#f7f9fc] p-5 text-center shadow-[inset_0_0_0_1px_rgba(37,99,235,.1)] sm:p-7">
        ...
        <div className="mx-auto mt-5 flex max-w-[250px] items-center gap-2 rounded-[9px] bg-white px-3 py-2 text-left shadow-[0_0_0_1px_rgba(15,23,42,.06)]">
...
function AnalysisPreview() {
  return (
    <div className="surface h-full w-full rounded-[17px] bg-white p-5 sm:p-6">
    ...
    <div className="surface h-full w-full rounded-[17px] bg-white p-5 sm:p-6">
      ...
      <div className="mt-4 divide-y divide-black/[.055]">
        {tasks.slice(0, compact ? 4 : 5).map((task, index) => (
          <div key={task} className="flex min-h-10 items-center gap-3 py-2">
        ...
        <div className="mt-16 grid items-stretch gap-12 lg:grid-cols-3 lg:gap-7">
          {steps.map((step) => (
            <article key={step.number} className="flex flex-col">
              ...
              <div className="mt-7 flex-1 lg:flex lg:min-h-[375px]">{step.visual}</div>
            </article>
          ))}
        </div>
      </div>
  ...
  return (
    <section id="features" className="bg-[#f4f6fa] pb-16 pt-12 sm:pb-20 sm:pt-12">
      <div className="mx-auto grid max-w-[1180px] items-center gap-14 px-5 lg:grid-cols-[.8fr_1.2fr] lg:gap-20">
        ...
        <div className="dashboard-shell rounded-[20px] bg-white p-2">
      ...
      <div className="mx-auto max-w-[1180px] px-5">
        <div className="grid items-center gap-14 lg:grid-cols-2 lg:gap-20">
          <div className="order-2 lg:order-1">
            <div className="surface rounded-[20px] bg-white p-4 sm:p-6">
              ...
              <div className="my-4 flex items-center gap-3"><span className="h-px flex-1 bg-black/[.07]" /><div className="flex items-center gap-2 rounded-full bg-[#edf3ff] px-3 py-1.5 text-[10px] font-semibold text-[#3d64c1]"><WandSparkles className="size-3.5" /> AI understanding</div><span className="h-px flex-1 bg-black/[.07]" /></div>
      ...
      <div className="mx-auto grid max-w-[1180px] items-center gap-14 px-5 lg:grid-cols-[.9fr_1.1fr] lg:gap-20">
        ...
        <div className="rounded-[20px] bg-white/[.055] p-2 shadow-[inset_0_0_0_1px_rgba(255,255,255,.1),0_28px_70px_-30px_rgba(0,0,0,.65)]">
          <div className="rounded-[14px] bg-[#f8f9fc] p-5 text-[#111a2e] sm:p-7">
            ...
            <div className="mt-6 space-y-2.5">{deadlines.map(([date, task, tone]) => <div key={task} className="surface-interactive flex items-center gap-4 rounded-[11px] bg-white p-4"><span className={`h-10 w-1 rounded-full ${tone === "red" ? "bg-[#d67d75]" : tone === "amber" ? "bg-[#d9a24b]" : "bg-[#5780df]"}`} /><div className="flex-1"><p className="text-[10px] font-semibold uppercase tracking-[.08em] text-[#8a92a0]">{date}</p><p className="mt-1 text-sm font-semibold text-[#354055]">{task}</p></div><StatusPill tone={tone}>{tone === "red" ? "Action required" : tone === "amber" ? "Due soon" : "Upcoming"}</StatusPill></div>)}</div>
  ...
  return (
    <section className="py-16 sm:py-20">
      <div className="mx-auto max-w-[1180px] px-5">
        ...
        <div className="dashboard-shell mx-auto mt-14 max-w-4xl rounded-[20px] bg-white p-2">
          <div className="overflow-hidden rounded-[14px] bg-[#f7f9fc]">
            ...
            <div className="mx-auto max-w-2xl space-y-5 p-4 sm:p-8">
              <div className="ml-auto max-w-[85%] rounded-[14px] rounded-br-[5px] bg-[#245eea] px-4 py-3 text-sm font-medium leading-6 text-white shadow-[0_7px_18px_-10px_rgba(37,99,235,.7)]">What should I take care of this week?</div>
              <div className="flex items-start gap-3"><span className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-[8px] bg-[#eaf1ff] text-[#4069cd]"><Sparkles className="size-3.5" /></span><div className="min-w-0 flex-1 rounded-[14px] rounded-tl-[5px] bg-white p-4 shadow-[0_0_0_1px_rgba(15,23,42,.06)]"><p className="text-sm font-semibold text-[#334054]">You have 3 priorities:</p><div className="mt-4 divide-y divide-black/[.055]">{[["1", "Submit your proposal revision", "Due today", "red"], ["2", "Complete your scholarship application", "Missing: Academic transcript", "amber"], ["3", "Pay your internet bill", "Due tomorrow", "blue"]].map(([number, title, detail, tone]) => <div key={number} className="flex gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-[#eef3ff] text-[10px] font-semibold text-[#4067c5]">{number}</span><div><p className="text-xs font-semibold text-[#3c475a]">{title}</p><p className={`mt-1 text-[10px] font-medium ${tone === "red" ? "text-[#b45e56]" : tone === "amber" ? "text-[#aa7524]" : "text-[#4a69b4]"}`}>{detail}</p></div></div>)}</div></div></div>
              <button className="ml-11 inline-flex min-h-10 items-center gap-2 rounded-[9px] bg-[#edf3ff] px-3 text-[11px] font-semibold text-[#3e64be] transition-[transform,background-color] duration-150 ease-out hover:bg-[#e4edff] active:scale-[0.96]"><Bell className="size-3.5" /> Create reminders for all deadlines</button>
            </div>
      ...
      <div className="mx-auto max-w-[1180px] px-5">
        ...
        <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {useCases.map(({ icon: Icon, title, items, tone }) => (
            <article key={title} className="surface-interactive rounded-[17px] bg-white p-6">
    ...
    <section className="py-16 sm:py-20">
      <div className="mx-auto max-w-[1180px] px-5">
        ...
        <div className="mt-10 grid gap-px overflow-hidden rounded-[18px] bg-black/[.07] shadow-[0_0_0_1px_rgba(15,23,42,.06)] sm:grid-cols-2 lg:grid-cols-4">
          {concepts.map(([value, label]) => <div key={value} className="bg-white p-7 text-center sm:p-9"><p className="text-balance text-3xl font-semibold tracking-[-.05em] text-[#17243e]">{value}</p><p className="text-pretty mt-2 text-xs leading-5 text-[#717a89]">{label}</p></div>)}
        </div>
