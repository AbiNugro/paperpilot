...
export function InboxPage() {
  ...
  return <div className="app-page-enter space-y-7"><SectionTitle eyebrow="Your workspace" title="Paper Inbox" copy="Every document that needs your attention, organized in one place." action={<PrimaryButton href="/upload"><Upload aria-hidden="true" className="size-4" />Upload document</PrimaryButton>} />
    ...
    <section aria-label="Documents list" className="space-y-2.5"><div className="hidden grid-cols-[minmax(0,1.7fr)_minmax(80px,.65fr)_minmax(110px,.8fr)_auto] gap-4 px-4 text-[10px] font-semibold uppercase tracking-[.12em] text-[#98a2b2] sm:grid"><span>Document</span><span>Deadline</span><span>Progress</span><span>Status</span></div>{filteredDocuments.length ? filteredDocuments.map((document) => <DocumentRow key={document.id} document={document} />) : <EmptyState title="No documents match that search." copy="Try a different search or filter, or upload a new document." />}</section>
    <p className="text-center text-[11px] text-[#909aaa]">{filteredDocuments.length} of {inboxDocuments.length} documents shown</p>
  </div>;
}
