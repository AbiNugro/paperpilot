...
import { documents, getDaysRemaining, getRequirementStats, getTaskStats, mockToday, openTasks, primaryDocument, upcomingDeadlineDocuments, type PaperTask } from "@/lib/mock-data";
...
export function Dashboard() {
  ...
  const nextDocuments = upcomingDeadlineDocuments.filter((document) => document.id !== primaryDocument.id).slice(0, 3);
  const highlightedDeadlineCount = upcomingDeadlineDocuments.slice(0, 3).length;
