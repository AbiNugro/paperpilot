...
export function DocumentDetail({ document }: { document: PaperDocument }) {
  ...
  return <div className="app-page-enter space-y-7">
    ...
    {activeTab === "original" ? <OriginalPlaceholder document={document} /> : <div role="tabpanel" id={`document-panel-${activeTab}`} aria-labelledby={`document-tab-${activeTab}`} tabIndex={0} className="grid gap-6 outline-none lg:grid-cols-[minmax(0,1.65fr)_minmax(270px,.8fr)]"><main className="space-y-6">
      {activeTab === "overview" && <>
      ...
      <section className="rounded-[16px] bg-white p-5 shadow-[0_0_0_1px_rgba(15,23,42,.065)] sm:p-6"><div className="flex items-center justify-between gap-3"><div><h2 className="text-base font-semibold text-[#2b3b57]">{t("nextActions")}</h2><p className="mt-1 flex flex-wrap gap-x-2 text-xs text-[#8590a2]"><span>{common("completedProgress", {completed: completedCount, total: tasks.length})}</span><span aria-hidden="true">·</span><span>{common("tasksRemaining", {count: tasks.length - completedCount})}</span></p></div><span className="tabular-nums text-xs font-semibold text-[#72809a]">{tasks.length ? Math.round((completedCount / tasks.length) * 100) : 0}%</span></div><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-[#edf1f6]"><div className="h-full rounded-full bg-[#5a7fdc] transition-[width] duration-200 ease-out" style={{ width: `${tasks.length ? (completedCount / tasks.length) * 100 : 0}%` }} /></div><div className="mt-4 divide-y divide-[#edf0f4]">{tasks.map((item) => <label key={item.id} className="flex min-h-11 cursor-pointer items-start gap-3 py-3 first:pt-0 last:pb-0"><input type="checkbox" className="sr-only" checked={item.status === "completed"} onChange={() => toggleTask(item.id)} /><TaskStatusIcon completed={item.status === "completed"} /><span className="min-w-0"><span className={`block text-pretty text-sm font-medium ${item.status === "completed" ? "text-[#8b95a4] line-through" : "text-[#3a4964]"}`}>{taskText(item, translateDocuments)}</span><span className="mt-1 flex items-center gap-1 text-[11px] text-[#8a95a4]"><DeadlineIcon />{item.dueDate ? common("due", {date: formatDate(item.dueDate, locale, "short")}) : common("status.completed")}</span></span></label>)}</div></section>
